...
def get_income(table: Table) -> Income:
...
def get_currency_operations(df: DataFrame) -> List[CurrencyOperation]:
    currency_operations: List[CurrencyOperation] = []

    for index, operation in df[
        df["Transaction description"].str.startswith("EB ", na=False)
    ].iterrows():
        currency: Currency = Currency(operation["Amount in original currency"][-3::])
        currency_amount: float = float(operation["Amount in original currency"][0:-4])
        ...
        currency_operations.append(
            CurrencyOperation(
                (currency_amount, currency), FinOp(title, date, amount), exchange_rate
            )
        )
...
def get_period(table: Table) -> Tuple[datetime, datetime]:
    df = table.dataframe
...
def get_top5_item_stat(df: DataFrame) -> List[Top5Payment]:
    ...
    for expense in top_5_rows.index:
        ...
        payments: Series = df.loc[df["Transaction description"] == expense]["Expense"]
...
class Aggregator:
    def generate_reports(self, tables: List[Table]) -> List[Report]:
        reports: List[Report] = []
        for table in tables:
            income: Income = get_income(table)
            outcome: Expenses = self.get_outcome(table)
            from_date, to_date = get_period(table)
            reports.append(
    ...
    def get_outcome(self, table: Table) -> Expenses:
        ...
        top_5_expenses: List[Top5Payment] = get_top5_item_stat(df)
        ...
        currency_operations: List[CurrencyOperation] = get_currency_operations(df)
